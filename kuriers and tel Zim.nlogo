globals [vsego x worktime i rest newkur kolzak k j timeobrzak timebtwnzak l labelkur dost zaktokur perem mylist ran]
breed [tels tel]
breed [kurs kur]
breed [zaks zak]
...
to setup
ca
  create-tels telephones
  ask tels [setxy random -9 random-ycor  set shape "telephone" set size 5 set label who + 1 set label-color black ]
  layout-circle sort tels max-pycor - 12

  create-kurs kuriers
  ask kurs [setxy 14 random-ycor set shape "person service" set size 3  set label who set label-color black]
  ask patches [set pcolor white]
  set labelkur telephones
  set k 0
...
to begin
  ...
  if count zaks > 1 [user-message (word "Курьер пошел работать" )
  ifelse k < kuriers [ask kurs with [label = telephones + k] [die] set k k + 1 ask zaks [die]]
    [ask kurs with [label = labelkur] [die] ]
    ask zaks [die]]
    set zaktokur zaktokur + 1

  ]
set mylist  [ 1 2 3 4]
set ran one-of mylist

  if (count zaks < telephones) and (timebtwnzak > 10) and (count kurs > 0)
  [set kolzak kolzak + 1
    create-zaks 1 [set shape "letter opened" set size 4 facexy 0 0  wait 0.5]]
  ;if count zaks > 0 [ask one-of zaks [fd 10] ]

  if count zaks > 0 [ask zaks with [who = zaktokur + telephones + kuriers + newkur ][fd 10 ]
  ;ask zaks with [who = zaktokur + telephones + kuriers + newkur + 1][fd 10 ]

  if count zaks = ran  [user-message (word "Курьер пошел работать" )
  ifelse k < kuriers [ask kurs with [label = telephones + k] [die] set k k + 1 ask zaks [die]]
    [ask kurs with [label =
labelkur] [die] ]
    ask zaks [die]]
...
if (count kurs < kuriers) and (dost > 15) [
    ...
    set newkur newkur + 1
    set labelkur labelkur + 1
   ]

if labelkur = telephones + kuriers [set labelkur telephones]
...
true
0
Rectangle -13840069 true false 75 30 225 150
Rectangle -7500403 true true 90 45 210 135
Rectangle -7500403 true true 75 180 225 240
Line -16777216 false 90 180 90 240
Line -16777216 false 105 180 105 240
Line -16777216 false 120 180 120 240
Line -16777216 false 135 180 135 240
Line -16777216 false 150 180 150 240
Line -16777216 false 165 180 165 240
Line -16777216 false 180 180 180 240
Line -16777216 false 195 180 195 240
Line -16777216 false 210 180 210 240
Line -16777216 false 75 195 225 195
Line -16777216 false 75 210 225 210
Line -16777216 false 75 225 225 225
Circle -13840069 true false 240 150 30
Line -16777216 false 240 165 270 165
Line -16777216 false 255 150 255 165
Rectangle -7500403 false true 120 150 180 165
Rectangle -7500403 true true 120 150 180 165
Rectangle -16777216 true false 90 45 210 135
...
true
6
Polygon -7500403 true false 149 1 115 108 2 108 93 175 58 282 149 216 241 282 206 175 297 108 184 108
